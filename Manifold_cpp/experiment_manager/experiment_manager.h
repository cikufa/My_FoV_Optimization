...
#include <monte_carlo_FIF.h>

#include <random>

class ExperimentManager{
public:
	ExperimentManager(float x_low,float x_high,float y_low,float y_high,float z_low,float z_high,std::string map_output_filename,
		int cluster_count,int feature_count_resolution, int map_count,int x_resolution, int y_resolution,int z_resolution,bool load_map,
		std::string map_name, bool generate_map_only, bool generate_direction_and_uncertainty){
...
TODO: /*map load/generate ----------------------------------------------------------------------------*/
	...
	void my_generate_random_map(const std::vector<int>& features_per_cluster){
		this->collection.clear();
		int number_of_clusters = features_per_cluster.size(); // Define clusters based on input vector
		generate_cluster_locations(number_of_clusters);
		
		for (int i = 0; i < number_of_clusters; i++){
			Eigen::Vector3f location = this->cluster_location[i];
			generate_random_cluster(location[0], location[1], location[2], features_per_cluster[i]); // Pass custom feature count
		}
	}
	...
	Eigen::Vector3f generate_cluster_location(){
		...
		float cluster_location_x=this->my_generate_uniform_number(x_limit[0],x_limit[1]);
		float cluster_location_y=this->my_generate_uniform_number(y_limit[0],y_limit[1]);
		float cluster_location_z=this->my_generate_uniform_number(z_limit[0],z_limit[1]);
		Eigen::Vector3f v;
		v<<cluster_location_x,cluster_location_y,cluster_location_z;
		return v;
	}
	
	float my_generate_uniform_number(float low, float high){
		std::random_device rd;
		std::default_random_engine generator(rd());
		std::uniform_real_distribution<float> distribution_uniform(low, high); // Changed from int to real
	...
	void generate_random_cluster(float x_mean,float y_mean,float z_mean,int sample_count){
		...
		float x_std=my_generate_uniform_number(15,30);
		float y_std=my_generate_uniform_number(15,30);
		float z_std=my_generate_uniform_number(15,30);

		float cluster_uncertainty=my_generate_uniform_number(1,80);

		float x_dir=my_generate_uniform_number(-30,30);
		float y_dir=my_generate_uniform_number(-30,30);
		float z_dir=0.0;		
